...
const topics = [
  {
    ...
    Content: () => (
      <>
        We start out in Chapter 1 by introducing the basics of GraphQL in
        contrast to REST. Then in Chapters 2–4, we go through the whole GraphQL
        specification from its basic building blocks. We also have an extensive
        Background chapter that covers everything from{' '}
        <Link to="/background/http/">HTTP</Link> and{' '}
        <Link to="/background/latency">latency</Link> to{' '}
        <Link to="/background/authentication/">authentication</Link> and{' '}
        <Link to="/background/databases/">databases</Link>.
      </>
    ),
  },
  {
  ...
  },
  {
    ...
    Content: () => (
      <>
        We’re working on a native iOS chapter that uses the Apollo iOS Swift
        client. Get your query and mutation results in query-specific Swift
        types, and have the data cached.
        <br />
        <i>
...
const Topic = ({ icon, title, Content }) => (
  <div className="Topic">
