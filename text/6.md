...
[`src/lib/auth.js`](https://github.com/GraphQLGuide/guide/blob/7_0.1.0/src/lib/auth.js)
...
[`src/lib/apollo.js`](https://github.com/GraphQLGuide/guide/compare/6_0.1.0...7_0.1.0)
...
[`src/lib/withUser.js`](https://github.com/GraphQLGuide/guide/blob/7_0.1.0/src/lib/withUser.js)
...
[`src/lib/auth.js`](https://github.com/GraphQLGuide/guide/compare/7_0.1.0...8_0.1.0)
...
[`src/lib/withUser.js`](https://github.com/GraphQLGuide/guide/compare/11_0.1.0...12_0.1.0)
...
[`src/components/Reviews.js`](https://github.com/GraphQLGuide/guide/compare/11_0.1.0...12_0.1.0)
...
> If you’re jumping in here, `git checkout 18_0.1.0` ([tag 18_0.1.0](https://github.com/GraphQLGuide/guide/tree/18_0.1.0)). Code from this section isn’t included in future tags.
...
We know there will always be a list of reviews in the store ordered by `createdAt_DESC` because that’s the default. We don’t know whether there will be a list ordered by `createdAt_ASC`. If there isn’t, `store.readQuery` will throw an error. So we’ll first read and write `DESC`, and then we’ll *try* read and writing `ASC`:

[`src/components/Review.js`](https://github.com/GraphQLGuide/guide/compare/18_0.1.0...19_0.1.0)
...
Also, if we can scroll through our entire list, then adding a new item should also go on the end of the Oldest list—not just the beginning of the Newest list as we’re doing currently. So let’s also write the new review to the `orderBy: 'createdAt_ASC'` list:

[`src/components/ReviewForm.js`](https://github.com/GraphQLGuide/guide/compare/18_0.1.0...19_0.1.0)
...
That wasn’t too hard—updating two queries when deleting and two when creating. But imagine if we had more sort orders—like last updated or number of stars—and added filters—like number of stars or sentences:
...
In total, there are `6 x 5 x 5 = 150` possibilities. That’s a lot of updating to do. Not only that, but we have to add in some logic—for instance, if a new review is submitted with 4 sentences and 4 stars, we don’t want to also add it any of the `minStars: 5` or `minSentences: 5` filtered lists. 
...
The first arg to the `WatchedMutationLink` constructor is our cache, and the second in an object that lists each mutation and the queries we want to update when that mutation is complete. In this case, the mutation named `DeleteReview` updates `ReviewsQuery` and `UserQuery`, and the `AddReview` mutation updates `ReviewsQuery`. At first, looking at our deletion update function, we might think to update `ReviewsQuery` and `ReadUserFavorites`: 
...
What we’re trying to update is the `favoriteCount` in the `<Reviews>` header, and that’s calculated from the `user` prop, which is fetched by the `UserQuery`, so that’s the attribute we’ll use in our config object. Here are the `ReviewsQuery` and `UserQuery` update functions filled in:
...
> If you’re jumping in here, `git checkout 18_0.1.0` ([tag 18_0.1.0](https://github.com/GraphQLGuide/guide/tree/18_0.1.0), or compare [18...19](https://github.com/GraphQLGuide/guide/compare/18_0.1.0...19_0.1.0))
...
[`src/lib/withUser.js`](https://github.com/GraphQLGuide/guide/compare/19_0.1.0...20_0.1.0)
...
[`src/lib/auth.js`](https://github.com/GraphQLGuide/guide/compare/19_0.1.0...20_0.1.0)
...
[`src/lib/apollo.js`](https://github.com/GraphQLGuide/guide/compare/19_0.1.0...20_0.1.0)
...
> If you’re jumping in here, `git checkout 19_0.1.0` ([tag 19_0.1.0](https://github.com/GraphQLGuide/guide/tree/19_0.1.0), or compare [19...20](https://github.com/GraphQLGuide/guide/compare/19_0.1.0...20_0.1.0))
...
Now Apollo will first look in the store for a `Section`’s `scrollY`, and then if it’s not there, fall back to the resolver that always returns zero.
...
> If you’re jumping in here, `git checkout 20_0.1.0` ([tag 20_0.1.0](https://github.com/GraphQLGuide/guide/tree/20_0.1.0), or compare [20...21](https://github.com/GraphQLGuide/guide/compare/20_0.1.0...21_0.1.0))
...
and get back a URL of an image, which it would return to us, which we would put in the `src` of an `<img>` tag:
...
Which one can we use? We don’t know the client’s city or GPS coordinates or zip code... so at the moment, none of them! There are a couple ways, though, to get the user’s location: 
...
The latter is more precise, easier to code, and gets the user’s consent via a built-in browser permission dialog. So let’s do that. All we need to do is just `navigator.geolocation.getCurrentPosition`, and after the user approves, we get the coordinates in a callback:
...
> If you’re jumping in here, `git checkout 21_0.1.0` ([tag 21_0.1.0](https://github.com/GraphQLGuide/guide/tree/21_0.1.0), or compare [21...22](https://github.com/GraphQLGuide/guide/compare/21_0.1.0...22_0.1.0))
...
The first subscription sends a response event when someone creates a new review. `reviewUpdated` fires whenever a review’s text or stars is edited, and `reviewDeleted` when one is deleted. For the first two, the events contain the review created/updated. For the last, it contains just the review’s id.
...
But our users usually won’t see messages logged to the console, so let’s think about how we want to display the new review notification to the user. We could `window.alert()`, but that requires dismissal and is annoying 😆. We could put it on the page—for example in the header—but then the notification would be stuck there page until either a new subscription event arrived or the page got rerendered. It doesn’t need to be shown for long, taking up the user’s brainspace and annoying them (at least *I’m* annoyed when I can’t dismiss a notification 😄). So let’s show a temporary message somewhere off to the side. We can search the Material UI [component library](https://material-ui.com/demos/app-bar/) and find the component meant for this purpose—the [Snackbar](https://material-ui.com/demos/snackbars/). We control whether it’s visible with an `open` prop, so we need state for that, and the `onClose` prop gets called when the user dismisses the Snackbar.
...
> If you’re jumping in here, `git checkout 22_0.1.0` ([tag 22_0.1.0](https://github.com/GraphQLGuide/guide/tree/22_0.1.0), or compare [22...23](https://github.com/GraphQLGuide/guide/compare/22_0.1.0...23_0.1.0))
...
> If you’re jumping in here, `git checkout 23_0.1.0` ([tag 23_0.1.0](https://github.com/GraphQLGuide/guide/tree/23_0.1.0), or compare [23...24](https://github.com/GraphQLGuide/guide/compare/23_0.1.0...24_0.1.0))

There are often more ways than just mouseovers to intelligently prefetch certain data. What the ways are depends on the type of app. We have to think about how the user uses the app, and what they might do next. In our app, one common action will probably be to read the next section. So a simple thing we can do is whenever we show a section, we always prefetch the next section. Let’s first get the `client` instance into `<Section>`. Before we used `withApollo()`, which we could do again, but we don’t need to—our `<Section>` is inside a `<Query>`, which always provides a client instance:
...
> If you’re jumping in here, `git checkout 24_0.1.0` ([tag 24_0.1.0](https://github.com/GraphQLGuide/guide/tree/24_0.1.0)). We won’t be leaving the code in this section in our app, so the next section will also start at tag `24`. 
...
> If you’re jumping in here, `git checkout 24_0.1.0` ([tag 24_0.1.0](https://github.com/GraphQLGuide/guide/tree/24_0.1.0), or compare [24...25](https://github.com/GraphQLGuide/guide/compare/24_0.1.0...25_0.1.0))
...
At this rate, we would fill up the cache after loading 5000 KB / 2 KB = 2500 sections, so 5 MB is currently plenty of room for us. So we can go with `localStorage`. 
...
On load, the app reads all the queries from the cache, including the `currentUser` query, which was saved to the cache when we logged in, and is still there! Along with all the private data we had access to, like bonus chapters. To fix this, we can clear the cache when we logout. In order to clear the cache, we need to use a different API from `apollo-cache-persist`. We’ve been using the basic API, `persistCache()`. The more advanced API is [`CachePersistor`](https://github.com/apollographql/apollo-cache-persist#advanced-usage):
...
> If you’re jumping in here, `git checkout 25_0.1.0` ([tag 25_0.1.0](https://github.com/GraphQLGuide/guide/tree/25_0.1.0), or compare [25...26](https://github.com/GraphQLGuide/guide/compare/25_0.1.0...26_0.1.0))
...
- If the endpoint is authenticated, we usually will want to keep those private on our server.
...
For building the `LAUNCH_QUERY`, let’s see what data is available from the API by browing its GraphiQL: [api.spacex.land/graphql/](https://api.spacex.land/graphql/). From the available queries, it looks like the relevant one for us is `launchNext`, and we can pick a few fields to display:
...
> If you’re jumping in here, `git checkout 26_0.1.0` ([tag 26_0.1.0](https://github.com/GraphQLGuide/guide/tree/26_0.1.0), or compare [26...27](https://github.com/GraphQLGuide/guide/compare/26_0.1.0...27_0.1.0))
...
We can see that a `__schema` has `types` that include an object with `name: "Query"` which a field named `sections` which has args `lastCreatedAt` and `limit`. And if we scroll down, we see more familiar fields and types.
...
> guide@0.1.0 lint /Users/me/gh/guide
> eslint src/
...
npm ERR! errno 1
npm ERR! guide@0.1.0 lint: `eslint src/`
npm ERR! Exit status 1
npm ERR!
npm ERR! Failed at the guide@0.1.0 lint script.
...
And now when we lint, we get one fewer error! Next set of errors is:
...
And `setSectionScroll` is our [local mutation](#local-mutations). ESLint will find neither of these in the Guide API schema. We can supress the errors by adding this line to the top of the file:
...
Git has [a lot of hooks](https://git-scm.com/docs/githooks)—times when git will run a program for you. One such hook is pre-commit. A pre-commit hook will be called when a dev enters `git commit` and before git actually does the committing. If the hook program ends with a non-0 exit code, the commit will be canceled. The best way to set up git hooks in our project is with [Husky](https://github.com/typicode/husky). To do that, we would:
...
Usually an app needs to process images or videos—for example resizing an image, centering on a face and cropping it, brightening, etc—before using them. For these apps, we recommend Cloudinary as the all-in-one solution. If you’re just saving files that need to be stored, and maybe downloaded later unchanged, then S3 is fine.
