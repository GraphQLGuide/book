...
We register a service in the same way we registered our monolith’s schema in [Analytics](../server/production/analytics.md) and [Schema validation](../server/schema-validation.md)—with the `apollo service:push` command:
...
To validate the service, we use `--serviceName` with the `apollo service:check` command we used in the [Schema validation](../server/schema-validation.md) section:
